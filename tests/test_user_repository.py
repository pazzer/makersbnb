...
def test_cannot_register_existing_email_address(db_connection):
    ...
    with pytest.raises(EmailAlreadyExistsError) as e:
        user_repository.create_user('alice@example.com', '12345678!')
    error_message = str(e.value)
...
def test_cannot_register_email_with_no_dotcom_at_end(db_connection):
    ...
    with pytest.raises(MalformedEmailError) as e:
        user_repository.create_user('alice@example.con', '12345678!')
    error_message = str(e.value)
...
def test_cannot_register_email_with_no_curly_at_character(db_connection):
    ...
    user_repository = UserRepository(db_connection)
    with pytest.raises(MalformedEmailError) as e:
        user_repository.create_user('alice_example.com', '12345678!')
    error_message = str(e.value)
...
def test_cannot_register_email_that_starts_with_curly_at_character(db_connection):
    ...
    user_repository = UserRepository(db_connection)
    with pytest.raises(MalformedEmailError) as e:
        user_repository.create_user('@example.com', '12345678!')
    error_message = str(e.value)
...
def test_can_register_valid_email_address(db_connection):
    db_connection.seed("seeds/makersbnb.sql")
    user_repository = UserRepository(db_connection)
    retval = user_repository.create_user('rupert@example.com', '12345678!')
...
def test_cannot_register_password_containing_whitespace(db_connection):
    ...
    with pytest.raises(MalformedPasswordError) as e:
        _ = user_repository.create_user('gottfried@go_mail.com', 'a23 456789!')
    error_message = str(e.value)
...
def test_cannot_register_password_without_special_character(db_connection):
    ...
    user_repository = UserRepository(db_connection)
    with pytest.raises(MalformedPasswordError) as e:
        _ = user_repository.create_user('gottfried@go_mail.com', 'a23456789')
    error_message = str(e.value)
...
def test_cannot_register_password_with_more_than_20_characters(db_connection):
    ...
    user_repository = UserRepository(db_connection)
    with pytest.raises(MalformedPasswordError) as e:
        _ = user_repository.create_user('gottfried@go_mail.com', '12345678!a12345678!a12345678!a')
    error_message = str(e.value)
...
def test_cannot_register_password_less_than_8_characters(db_connection):
    ...
    user_repository = UserRepository(db_connection)
    with pytest.raises(MalformedPasswordError) as e:
        _ = user_repository.create_user('gottfried@go_mail.com', 'a23456!')
    error_message = str(e.value)
...
def test_can_register_valid_password(db_connection):
    db_connection.seed("seeds/makersbnb.sql")
    user_repository = UserRepository(db_connection)
    assert user_repository.create_user('gottfried@go_mail.com', '12345678!') is None
...
def test_check_password_rejects_wrong_password(db_connection):
    ...
    assert not user_repository.find_by_email_and_password('bob@example.com', '_@qwerty456')
    assert not user_repository.find_by_email_and_password('carol@example.com', 'securepass789$$')
...
def test_check_password_accepts_correct_password(db_connection):
    ...
    assert user_repository.find_by_email_and_password('bob@example.com', '_@qwerty456£')
    assert user_repository.find_by_email_and_password('carol@example.com', 'securepass789$$^')
    assert user_repository.find_by_email_and_password('dave@example.com', '&&let&mei&n321')
    assert user_repository.find_by_email_and_password('eve@example.com', 'a@dmi£n1234')
